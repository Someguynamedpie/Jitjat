...
static void test_number(const char *file, unsigned int line, lua_State *L, lua_Number v, size_t n) {
    size_t len;
...
verify:
    size_is(file, line, len, n, "number");

teardown:
    lua_pop(L, 1);
}
#define TEST_NUMBER(v, n)       test_number(__FILE__, __LINE__, L, v, n)
